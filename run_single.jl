...
default(
    fontfamily="Computer Modern", # Or "serif"
    linewidth=1.5,
    framestyle=:box,
...
# ──────────────────────────────────────────────────────────
if length(ARGS) < 1
    println("Usage: julia run_single.jl <case.toml>")
    exit(1)
end
case_file = ARGS[1]
...
for j in 1:Z
    ...
    for i in 1:n_t
        ...
        # Inner GC (mirroring kernel.jl with thermal expansion + tilt)
...
for i in 1:n_t
    ...
    for j in 1:Z
        ...
        # Groove center inner (mirroring kernel.jl with thermal expansion + tilt)
        ...
        δr_or_q = CTE_q * (T_or_q - T_ref_q) * (D_o_star / 2)
        tilt_q = cθ * γy_q + sθ * γz_q
        R_pi_q = D_i_star / 2 + δr_ir_q
        r_gc_i = R_pi_q + y_ir_s * (-sθ) + z_ir_s * cθ - x_gi0_star * tilt_q
        x_gc_i = x_gi0_star + x_ir_s + R_pi_q * tilt_q

        # Groove center outer (with thermal expansion)
        r_gc_o = D_o_star / 2 + δr_or_q
        x_gc_o = x_go0_star
...
fig07_p1 = plot(title="Q_i — Inner Race Contact Load (N)", ylabel="", grid=true, gridalpha=0.3)
fig07_p2 = plot(title="Q_o — Outer Race Contact Load (N)", ylabel="", grid=true, gridalpha=0.3)
fig07_p3 = plot(title="α_i — Inner Contact Angle (°)", ylabel="", xlabel="Time (ms)", grid=true, gridalpha=0.3)
fig07_p4 = plot(title="α_o — Outer Contact Angle (°)", ylabel="", xlabel="Time (ms)", grid=true, gridalpha=0.3)
...
println("  --> [DEBUG] M_gyro max: ", maximum(fo_M_gyro), " min: ", minimum(fo_M_gyro), " NaNs: ", count(isnan.(fo_M_gyro)))
println("  --> [DEBUG] MgD max: ", maximum(fo_MgD), " min: ", minimum(fo_MgD), " NaNs: ", count(isnan.(fo_MgD)))
println("  --> [DEBUG] LAST 5 M_gyro: ", fo_M_gyro[end-4:end, 1])
println("  --> [DEBUG] LAST 5 omega_mag: ", fo_omega_mag[end-4:end, 1])
println("  --> [DEBUG] LAST 5 theta_dot: ", fo_theta_dot[end-4:end, 1])
println("  --> [DEBUG] LAST 5 I_ball: ", I_ball)
...
let y_stack = zeros(n_t)
    for (vals, label, color) in heat_sources
        y_new = y_stack .+ abs.(vals)  # abs for stacked display
...
for (label, data, color) in source_labels_data
    ...
    bar!(fig13, balls_x, vals, fillrange=bar_bottom, label=label, color=color, alpha=0.85)
    bar_bottom .+= vals
end
...
# ── Plot 14: Thermal Response ──
th_off = ADORE.thermal_offset(Z)
if size(u_mat, 2) >= th_off + 3
    T_ir_C = u_mat[:, th_off] .- 273.15
    T_or_C = u_mat[:, th_off+1] .- 273.15
    T_ball_C = u_mat[:, th_off+2] .- 273.15
    T_oil_C = u_mat[:, th_off+3] .- 273.15

    fig14 = plot(title="Thermal Network Temperature Response", xlabel="Time (ms)",
        ylabel="Temperature (°C)", grid=true, gridalpha=0.3, legend=:topleft, size=(1000, 600))
    plot!(fig14, t_ms, T_ir_C, color=:steelblue, lw=2, label="Inner Race")
    plot!(fig14, t_ms, T_ball_C, color=:darkred, lw=2, label="Balls (Lumped)")
    plot!(fig14, t_ms, T_or_C, color=:orange, lw=2, label="Outer Race")
    plot!(fig14, t_ms, T_oil_C, color=:darkgreen, lw=2, linestyle=:dash, label="Lubricant / Sump")

    savefig(fig14, joinpath(out_dir, "14_Temperatures.png"))
    println("  [OK] 14_Temperatures.png")
end
...
# Thermal node temperatures at start and end
th_off_idx = ADORE.thermal_offset(Z)
T_start = u_mat[1, th_off_idx:th_off_idx+3]
T_end = u_mat[end, th_off_idx:th_off_idx+3]

# Dimensional scales
P_scale = scales.Q * scales.L / scales.T  # Power scale [W]
...
ΔT = T_end .- T_start
E_stored = C_ir_dim * ΔT[1] + C_or_dim * ΔT[2] + C_ball_dim * ΔT[3] + C_oil_dim * ΔT[4]

# 2. All kernel energy terms from ODE accumulator DOFs (exact)
ha_off_idx = ADORE.heat_accum_offset(Z)
E_ir_nd = u_mat[end, ha_off_idx]
E_or_nd = u_mat[end, ha_off_idx+1]
E_ball_nd = u_mat[end, ha_off_idx+2]
E_oil_nd = u_mat[end, ha_off_idx+3]
E_conv_nd = u_mat[end, ha_off_idx+4]   # ∫Q_to_amb dτ

# Nondim → Dimensional conversion (E_dim = E_nd × P_scale × T_scale)
ndim_to_J = P_scale * scales.T  # P_scale = G_scale = Q·L/T, so same for all accumulators
E_ir_dim = E_ir_nd * ndim_to_J
E_or_dim = E_or_nd * ndim_to_J
E_ball_dim = E_ball_nd * ndim_to_J
E_oil_dim = E_oil_nd * ndim_to_J
E_conv_dim = E_conv_nd * ndim_to_J  # G★·ΔT has same nondim units as H★

E_kernel_dim = E_ir_dim + E_or_dim + E_ball_dim + E_oil_dim

# 3. ODE-consistent energy balance (all terms from same ODE solver path)
# accum[5] = ∫Σ C★·dT/dτ dτ  (ODE-integrated stored energy change)
# Note: Σ C·ΔT from state endpoints ≠ ∫C·dT/dτ dτ due to FBDF Newton residual drift.
# The ODE-integrated value provides exact consistency with the heat accumulators.
E_stored_ode = u_mat[end, ha_off_idx+5] * ndim_to_J
E_balance = E_kernel_dim - E_stored_ode - E_conv_dim
relative_err = E_kernel_dim > 0 ? abs(E_balance) / E_kernel_dim * 100 : 0.0

# 4. Field_output's independent estimate
dt_vec = diff(t_dim)
E_generated_fo = sum(0.5 .* (H_bearing_total[1:end-1] .+ H_bearing_total[2:end]) .* dt_vec)
model_discrepancy = E_kernel_dim > 0 ? abs(E_generated_fo - E_kernel_dim) / E_kernel_dim * 100 : 0.0

@printf("    Simulation duration  : %.3f s\n", t_dim[end])
@printf("    Initial T (IR/OR/Ball/Oil) : [%.1f, %.1f, %.1f, %.1f] K\n", T_start...)
@printf("    Final   T (IR/OR/Ball/Oil) : [%.1f, %.1f, %.1f, %.1f] K\n", T_end...)
@printf("    ΔT                         : [%.3f, %.3f, %.3f, %.3f] K\n", ΔT...)
@printf("\n")
@printf("    Thermal Capacities [J/K]:\n")
@printf("      C_ir=%.2f, C_or=%.2f, C_ball=%.2f, C_oil=%.2f\n", C_ir_dim, C_or_dim, C_ball_dim, C_oil_dim)
@printf("\n")
@printf("    E_kernel  (∫H_kernel dt) : %10.3f J  (from ODE accumulator DOFs)\n", E_kernel_dim)
@printf("      ├ Inner race heat      : %10.3f J\n", E_ir_dim)
@printf("      ├ Outer race heat      : %10.3f J\n", E_or_dim)
@printf("      ├ Ball heat            : %10.3f J\n", E_ball_dim)
@printf("      └ Oil (drag+churn)     : %10.3f J\n", E_oil_dim)
@printf("    E_stored  (Σ CΔT)        : %10.3f J  (state vector)\n", E_stored)
@printf("    E_stored  (ODE accum)    : %10.3f J  (∫Σ CdT/dτ dτ)\n", E_stored_ode)
@printf("    E_convected (ODE accum)  : %10.3f J\n", E_conv_dim)
@printf("    ─────────────────────────────────────\n")
@printf("    E_balance                 : %10.6f J  (kernel - stored_ode - convected)\n", E_balance)
@printf("    ODE Conservation error   : %10.6f %%\n", relative_err)
if relative_err < 0.1
    println("    ✅ Energy conservation EXACT (< 0.1%)")
else
    @printf("    ⚠️  ODE conservation error: %.4f%%\n", relative_err)
end
@printf("\n")
@printf("    E_field_output (∫H_fo dt) : %10.3f J\n", E_generated_fo)
@printf("    Model discrepancy (fo/ker): %8.4f %%\n", model_discrepancy)
@printf("    (kernel: 1D κ·Q traction; field_output: 2D TEHD integral)\n")
