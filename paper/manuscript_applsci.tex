...
where $\Upsilon$ is the Hertzian contact stiffness (computed from the curvature sum and elliptic integral parameters) and $\delta_+$ is a smoothed positive penetration function:
...
This ensures $C^\infty$ continuity at the contact--separation boundary, preventing numerical difficulties during transient contact events. All contact stiffness coefficients are precomputed from the bearing geometry and material modulus.
...
where the last term captures the differential sliding due to the curvature mismatch between ball and groove. The traction coefficient is obtained from a four-parameter isothermal traction curve (Table~\ref{tab:traction}); the parameter values are representative of polyol-ester lubricants and are taken from the ranges reported in the bearing dynamics literature~\cite{Gupta1984,Harris2006}---they are \emph{not} fitted to the NASA 35-mm thermal data used for validation. The traction coefficient is then corrected sequentially for (i)~Bair--Winer pressure-proportional limiting shear stress truncation~\cite{Bair1979} and (ii)~Archard flash temperature rise with thermal softening (floored at~25\%).
...
A four-node lumped-parameter thermal network (LPTN) is integrated into the ODE state vector. The energy balance for each node takes the form:
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{CCC}
        ...
        \textbf{Parameter} & \textbf{Symbol} & \textbf{Value} \\
        \midrule
        Initial traction coefficient & $\kappa_0$ & 0.010 \\
        Asymptotic traction coefficient & $\kappa_\infty$ & 0.005 \\
        Peak traction coefficient & $\kappa_m$ & 0.10 \\
        Slip velocity at peak & $u_m$ & \SI{0.5}{m/s} \\
...
\begin{table}[H]
    \caption{Heat generation comparison: simulation vs.\ NASA TP-2275 (35-mm bore, $F_a = 667$~N, $T_{\text{oil,in}} = 394$~K).\label{tab:validation}}
    \begin{tabularx}{\textwidth}{CCCCC}
        \toprule
        \textbf{$n$ (kRPM)} & \textbf{DN ($\times 10^6$)} & \textbf{NASA $H$ (W)} & \textbf{Sim.\ $H$ (W)} & \textbf{Error (\%)} \\
        \midrule
        28 & 0.98 & 350 & 341 & $-2.6$ \\
        48 & 1.68 & 700 & 680 & $-2.9$ \\
        64 & 2.24 & 1100 & 1134 & $+3.0$ \\
        72 & 2.52 & 1400 & 1400 & $-0.0$ \\
        \midrule
        \multicolumn{4}{c}{Mean absolute error} & 2.1\% \\
        \bottomrule
    \end{tabularx}
\end{table}

The power-law speed scaling exponent [$H \propto n^\alpha$] is $\alpha_{\text{NASA}} = 1.47$ and $\alpha_{\text{sim}} = 1.50$, confirming that the model correctly captures the relative contributions of speed-linear rolling contact traction and speed-squared churning drag.
...
Table~\ref{tab:temperatures} presents the predicted steady-state temperatures at the four NASA speed points. Although NASA TP-2275 does not report per-node temperature data for the 35-mm bearing, the predicted inner--outer race differential ($\Delta T_{\text{IR-OR}} \approx 5$--8~K) and the ranking $T_{\text{ball}} > T_{\text{IR}} > T_{\text{OR}} > T_{\text{oil}}$ are consistent with the general observations in bearing thermal literature~\cite{Takabi2013,Zheng2018}.
...
The initial diametral clearance $P_d$ is swept over six values: 0, 5, 10, 15, 20, and 30~$\mu$m.

\begin{figure}[H]
    \centering
    \includegraphics[width=12.0cm]{figures/clearance_transient_overlay.png}
    \caption{Transient inner race temperature for different initial diametral clearances (50,000~RPM, $F_a = 667$~N, $T_{\text{oil,in}} = 394$~K).\label{fig:clearance_transient}}
\end{figure}
...
\textbf{Result}: within the present model formulation, no measurable thermal sensitivity to the tested clearance range (0--30~$\mu$m) was observed under the specified preload condition. All six cases converge to identical steady-state temperatures and heat generation within numerical precision.

Under a 667~N axial thrust, the bearing operates in a fully preloaded regime: all 16~balls are in contact at all times, the contact angles adjust to accommodate the load regardless of initial clearance, and the resulting load distribution and sliding velocities are virtually identical across the clearance range. The 0--30~$\mu$m clearance variation is much smaller than the axial displacement imposed by the 667~N load ($\delta_a \approx 20$--40~$\mu$m), so the additional clearance is immediately consumed by the axial equilibrium.

To test whether this insensitivity persists under lighter loading, the sweep was repeated at $F_a = 100$~N with an extended clearance range of 0--100~$\mu$m. The result was identical: no detectable temperature variation across the entire range. This confirms that the observed insensitivity arises at least in part from the model's treatment of clearance as an initial quasi-static parameter; once the dynamic simulation begins, the contact geometry evolves through the 6-DOF force balance and thermal expansion independently of $P_d$. The current four-node LPTN cannot resolve clearance-dependent load-zone effects in the thermal feedback loop. In the physical system, larger clearances under light preload would produce partial load zones with fewer loaded elements and different temperature distributions---an effect that requires circumferentially resolved thermal modeling.

\textbf{Model limitation}: the clearance insensitivity reported here should be interpreted as an observation within the present model structure, not as a universal physical conclusion. Two factors contribute: (a)~the four-node LPTN averages all ball temperatures, making it structurally unable to resolve circumferential heat distribution shifts caused by partial load zones (see Section~\ref{sec:lptn_limit}); and (b)~under axial preload, the Hertzian contact stiffness dominates the equilibrium geometry such that small clearance changes produce negligible shifts in contact angle and sliding velocity. Different behavior would be expected with a circumferentially resolved thermal model under (i)~light preload with partial contact, (ii)~larger clearances ($> 50~\mu$m), or (iii)~predominantly radial loading.
...
\begin{figure}[H]
    ...
    \caption{Heat source breakdown vs.\ load angle, showing all seven individual contributions: inner and outer race sliding ($H_{\text{slide,i}}$, $H_{\text{slide,o}}$), inner and outer race spin ($H_{\text{spin,i}}$, $H_{\text{spin,o}}$), translational cage drag ($H_{\text{drag}}$), churning ($H_{\text{churn}}$), and total ($H_{\text{total}}$). The spin$_i$ and churning components are negligibly small under the present operating conditions (see text). Conditions: same as Figure~\ref{fig:loadratio_temp}.\label{fig:loadratio_heat}}
\end{figure}
...
The parametric trends reported in this study apply specifically to:
\begin{itemize}
    \item The NASA 35-mm bore ACBB geometry (Table~\ref{tab:geometry}).
    \item The DN range $1.0$--$2.5 \times 10^6$.
